...
def _normalize_zone_id(raw: str) -> str:
    ...
    data_zone_match = re.search(r"data-zone=['\"]?(\d+)", raw)
    if data_zone_match:
        return data_zone_match.group(1)

    numeric_match = re.search(r"(\d{4,})", raw)
    if numeric_match:
...
async def verification_page(request: web.Request) -> web.Response:
    ...
    <head>
      ...
      {"<script src='https://libtl.com/sdk.js' data-zone='" + zone_id + "' data-sdk='show_" + zone_id + "'></script>" if zone_id else ""}
      <style>
      ...
      <script>
        (function() {{
          ...
          async function waitForSdk(maxMs) {{
            if (!zoneId) throw new Error('zone_not_configured');
          ...
          async function startFlow() {{
            ...
            try {{
              ...
              if (zoneId) {{
                ...
                if (adResult === true) {{
                  adOk = true;
                }} else if (typeof adResult === 'string') {{
                  adOk = adResult.toLowerCase().includes('reward') || adResult.toLowerCase().includes('complete') || adResult.toLowerCase() === 'ok';
                }} else if (adResult && typeof adResult === 'object') {{
                  const status = String(adResult.status || adResult.state || '').toLowerCase();
                  adOk = Boolean(adResult.completed || adResult.rewarded || status.includes('reward') || status.includes('complete') || status === 'ok' || status === 'success');
                }}
